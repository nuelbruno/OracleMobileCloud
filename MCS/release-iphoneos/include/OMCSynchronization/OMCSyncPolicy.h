...
/**
 *  Fetches from the service if there is a cache miss or the cached item has expired.
 *  If hit cache, schedules a refresh in the background. Returns an error if offline and fetching from the service.
 *  On cache miss, this policy behaves just like FETCH_POLICY_FETCH_FROM_SERVICE_ON_CACHE_MISS_OR_EXPIRY.
 *  On cache hit, app will get result from cache, plus schedule a background refresh.
 */
...
/**
 An `SyncPolicy` object is used to specify the synchronization policies for a ResourceObject, ResourceCollection or ResourceFile. The policy is persisted along with the cached objects and is in effect even after application restarts.
 */
...
/**
 The time after which the cache expires.
 */
@property(nonatomic, readwrite) int32_t expire_After;

/**
 The boolean to specify not to cache.
 */
@property(nonatomic, readwrite) BOOL no_cache;


/**
 * Creates a new SyncPolicy object with specified policies.
 * See the corresponding getters and setters for more details.
...
/**
 * Returns a default OMCSyncPolicy object.
 *
 */
